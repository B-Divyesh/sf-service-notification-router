...
pub async fn status(State(state): State<AppState>) -> Result<Json<Value>> {
    let initialized = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM settings")
...
pub async fn setup(
    ...
    if sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM settings")
        .fetch_one(&state.pool)
        .await?
        > 0
    {
    ...
    if input.password.chars().count() < 12 {
        return Err(AppError::BadRequest(
            "Use an admin password of at least 12 characters.".into(),
        ));
    }
    let retention = input.retention_hours.unwrap_or(72);
    if !(1..=720).contains(&retention) {
        return Err(AppError::BadRequest(
            "Retention must be between 1 and 720 hours.".into(),
        ));
    }
    let salt = SaltString::generate(&mut OsRng);
    let password_hash = Argon2::default()
        .hash_password(input.password.as_bytes(), &salt)
        .map_err(|_| AppError::Internal("The admin password could not be secured.".into()))?
        .to_string();
    ...
    Ok((
        StatusCode::CREATED,
        Json(json!({"token":session,"webhook_secret":webhook_secret})),
    ))
}
...
pub async fn login(
    ...
    let hash = sqlx::query_scalar::<_, String>("SELECT password_hash FROM settings WHERE id=1")
        .fetch_optional(&state.pool)
        .await?
        .ok_or(AppError::NotFound("Set up the router first.".into()))?;
    let parsed = PasswordHash::new(&hash)
        .map_err(|_| AppError::Internal("The stored password is invalid.".into()))?;
    if Argon2::default()
...
pub async fn config(State(state): State<AppState>, headers: HeaderMap) -> Result<Json<Value>> {
    ...
    let recipient_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM recipients")
        .fetch_one(&state.pool)
        .await?;
    let rule_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM rules")
        .fetch_one(&state.pool)
        .await?;
    Ok(Json(json!({
...
pub async fn update_config(
    ...
    require_auth(&state, &headers).await?;
    validate_name(&input.business_name, "Business name")?;
    if !(1..=720).contains(&input.retention_hours) {
        return Err(AppError::BadRequest(
            "Retention must be between 1 and 720 hours.".into(),
        ));
    }
    sqlx::query("UPDATE settings SET business_name=?, retention_hours=? WHERE id=1")
...
pub async fn rotate_secret(
    ...
    sqlx::query("UPDATE settings SET webhook_secret=? WHERE id=1")
        .bind(&secret)
        .execute(&state.pool)
        .await?;
    Ok(Json(
        json!({"webhook_secret":secret,"notice":"Update the sender before its next booking; the previous secret no longer works."}),
    ))
}
...
pub async fn create_recipient(
    State(state): State<AppState>,
    headers: HeaderMap,
    Json(input): Json<RecipientInput>,
) -> Result<(StatusCode, Json<Value>)> {
    require_auth(&state, &headers).await?;
    ...
    if !matches!(input.channel.as_str(), "email" | "webhook") {
        return Err(AppError::BadRequest(
            "Channel must be email or webhook.".into(),
        ));
    }
    if !input.consent_confirmed {
        return Err(AppError::BadRequest(
            "Confirm this recipient agreed to receive operational notices.".into(),
        ));
    }
    if input.channel == "email" {
        if !input.destination.contains('@') || input.destination.contains(char::is_whitespace) {
            return Err(AppError::BadRequest("Enter a valid email address.".into()));
        }
    } else {
        let url = reqwest::Url::parse(&input.destination)
            .map_err(|_| AppError::BadRequest("Enter a complete webhook URL.".into()))?;
...
pub async fn delete_recipient(
    ...
    require_auth(&state, &headers).await?;
    let result = sqlx::query("DELETE FROM recipients WHERE id=?")
        .bind(id)
        .execute(&state.pool)
        .await?;
    if result.rows_affected() == 0 {
        Err(AppError::NotFound("Recipient not found.".into()))
    } else {
...
pub async fn create_rule(
    ...
    if !matches!(input.match_field.as_str(), "service" | "provider") {
        return Err(AppError::BadRequest(
            "Match field must be service or provider.".into(),
        ));
    }
    validate_name(&input.match_value, "Match value")?;
    let exists =
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM recipients WHERE id=? AND active=1")
            .bind(input.recipient_id)
            .fetch_one(&state.pool)
            .await?
            > 0;
    if !exists {
        return Err(AppError::BadRequest("Choose an active recipient.".into()));
    }
...
pub async fn delete_rule(
    ...
    let result = sqlx::query("DELETE FROM rules WHERE id=?")
        ...
        .await?;
    if result.rows_affected() == 0 {
        Err(AppError::NotFound("Rule not found.".into()))
    } else {
        Ok(StatusCode::NO_CONTENT)
    }
}
...
pub async fn receive_booking(
    ...
    let secret = sqlx::query_scalar::<_, String>("SELECT webhook_secret FROM settings WHERE id=1")
        .fetch_optional(&state.pool)
        .await?
        .ok_or(AppError::NotFound("Set up the router first.".into()))?;
    ...
    let booking: BookingInput = serde_json::from_slice(&body)
        .map_err(|_| AppError::BadRequest("Send a valid normalized booking JSON body.".into()))?;
    ingest_booking(&state, booking).await
}

pub async fn test_booking(
    State(state): State<AppState>,
    headers: HeaderMap,
    Json(booking): Json<BookingInput>,
) -> Result<(StatusCode, Json<Value>)> {
    require_auth(&state, &headers).await?;
    ingest_booking(&state, booking).await
}
...
async fn ingest_booking(
    ...
    if let Some(existing) =
        sqlx::query_scalar::<_, String>("SELECT id FROM bookings WHERE external_id=?")
            ...
            .await?
    {
        return Ok((
            StatusCode::OK,
            Json(json!({"accepted":true,"duplicate":true,"booking_id":existing})),
        ));
    }
    ...
    let matched = rules.into_iter().find(|row| {
        ...
        let actual = if field == "service" {
            Some(booking.service.as_str())
        } else {
            booking.provider.as_deref()
        };
        actual.is_some_and(|v| v.eq_ignore_ascii_case(expected.trim()))
    });
    let id = Uuid::new_v4().to_string();
    let serialized =
        serde_json::to_vec(&booking).map_err(|error| AppError::Internal(error.to_string()))?;
    ...
    if let Some(rule) = matched {
        ...
        let status = sqlx::query_scalar::<_, String>("SELECT status FROM notifications WHERE id=?")
        ...
        Ok((
            StatusCode::ACCEPTED,
            Json(
                json!({"accepted":true,"booking_id":id,"matched":false,"delivery_status":"unmatched"}),
            ),
        ))
    }
}
...
fn validate_booking(input: &BookingInput) -> Result<()> {
    ...
    if input.external_id.len() > 160 || input.service.len() > 160 {
        return Err(AppError::BadRequest(
            "External ID and service must be 160 characters or fewer.".into(),
        ));
    }
    if let Some(email) = &input.customer_email {
        if email.len() > 254 {
            return Err(AppError::BadRequest("Customer email is too long.".into()));
        }
    }
    Ok(())
}
...
pub async fn list_events(State(state): State<AppState>, headers: HeaderMap) -> Result<Json<Value>> {
    ...
    let received = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM bookings")
        .fetch_one(&state.pool)
        .await?;
    let delivered = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM notifications WHERE status IN ('delivered','acknowledged')",
    )
    .fetch_one(&state.pool)
    .await?;
    ...
    .await?;
    Ok(Json(
        json!({"events":events,"metrics":{"received":received,"delivered":delivered,"acknowledged":acknowledged,"unmatched":unmatched}}),
    ))
}
...
pub async fn retry_event(
    ...
    require_auth(&state, &headers).await?;
    let exists = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM notifications WHERE id=?")
        .bind(id)
        .fetch_one(&state.pool)
        .await?
        > 0;
...
pub async fn ack_status(
    ...
    Ok(Json(
        json!({"service":row.get::<String,_>("service"),"starts_at":row.get::<Option<String>,_>("starts_at"),"status":row.get::<String,_>("status"),"acknowledged_at":row.get::<Option<String>,_>("acknowledged_at")}),
    ))
}
...
pub async fn acknowledge(
    ...
    if result.rows_affected() == 0 {
        return Err(AppError::NotFound(
            "This acknowledgment link is invalid or no longer available.".into(),
        ));
    }
...
pub async fn purge_expired(state: &AppState) -> Result<u64> {
    let retention = sqlx::query_scalar::<_, i64>("SELECT retention_hours FROM settings WHERE id=1")
        .fetch_optional(&state.pool)
        .await?
        .unwrap_or(72);
...
pub async fn activate_license(
    ...
    require_auth(&state, &headers).await?;
    if input.token.len() < 12 || input.token.len() > 2048 {
        return Err(AppError::BadRequest(
            "Enter the complete license token.".into(),
        ));
    }
    let url = format!(
    ...
    let response = state
        .http
        .get(url)
        .query(&[("license", &input.token)])
        .send()
        .await
        .map_err(|_| {
            AppError::BadRequest(
                "The license service could not be reached. Your free routes still work.".into(),
            )
        })?;
    if !response.status().is_success() {
        return Err(AppError::BadRequest(
            "The license service did not accept that verification request.".into(),
        ));
    }
    let verdict: Value = response.json().await.map_err(|_| {
        AppError::BadRequest("The license service returned an unreadable response.".into())
    })?;
    let valid = verdict
        .get("valid")
    ...
    if valid {
        ...
        sqlx::query(
            "UPDATE settings SET licensed=1,license_token_enc=?,license_checked_at=? WHERE id=1",
        )
        .bind(encrypted)
        .bind(Utc::now().to_rfc3339())
        .execute(&state.pool)
        ...
        sqlx::query(
            "UPDATE settings SET licensed=0,license_token_enc=NULL,license_checked_at=? WHERE id=1",
        )
        .bind(Utc::now().to_rfc3339())
        .execute(&state.pool)
        .await?;
