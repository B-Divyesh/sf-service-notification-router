...
pub async fn deliver_notification(state: &AppState, notification_id: i64) {
    if let Err(error) = deliver(state, notification_id).await {
        ...
        let attempts =
            sqlx::query_scalar::<_, i64>("SELECT attempt_count FROM notifications WHERE id = ?")
        ...
        let _ = sqlx::query(
            "UPDATE notifications SET status='failed', error=?, next_attempt_at=? WHERE id=?",
        )
...
async fn deliver(state: &AppState, notification_id: i64) -> anyhow::Result<()> {
    ...
    if row.get::<String, _>("status") == "acknowledged" {
        return Ok(());
    }
    ...
    if encrypted.is_empty() {
        anyhow::bail!("booking details have already been purged");
    }
    let payload: BookingInput =
        serde_json::from_slice(&crypto::decrypt(&state.encryption_key, &encrypted)?)?;
    ...
    let response_code = if channel == "webhook" {
        ...
        if !status.is_success() {
            anyhow::bail!("recipient webhook returned HTTP {}", status.as_u16());
        }
...
async fn send_email(
    ...
    let host =
        state.config.smtp_host.as_deref().ok_or_else(|| {
            anyhow::anyhow!("SMTP is not configured; set SMTP_HOST and SMTP_FROM")
        })?;
    ...
    let customer = booking
        .customer_name
        .as_deref()
        .unwrap_or("Customer name not supplied");
    ...
    let email = Message::builder()
        .from(from)
        .to(to)
        .subject(format!("Booking to coordinate: {}", booking.service))
        .body(body)?;
    let mut builder =
